...
<aside class="w-64 flex-shrink-0 bg-gray-800 text-gray-300 flex flex-col shadow-lg">
    ...
    <nav class="flex-1 overflow-y-auto mt-4 space-y-1 px-2 text-sm" x-data="{}">
        ...
        <!-- Order Management -->
        <div x-data="{ isOpen: <?php echo isMenuOpen(['orders', 'order-categories'], $currentPath); ?> }">
            <button @click="isOpen = !isOpen" class="w-full flex justify-between items-center py-2.5 px-4 rounded-lg transition duration-200 <?php echo isMenuOpen(['orders', 'order-categories'], $currentPath) === 'true' ? 'text-white' : $inactiveParentClass; ?>">
            ...
            <div x-show="isOpen" x-transition class="mt-1 space-y-1 pr-4 border-r-2 border-gray-600">
                ...
                <!-- لینک جدید برای دسته‌بندی سفارشات -->
                <a href="<?php echo APP_URL; ?>/order-categories" class="block py-2 px-3 rounded-md <?php echo isActiveLink('order-categories', $currentPath) ? $activeChildClass : $inactiveChildClass; ?>">دسته‌بندی‌ها</a>
            </div>
        </div>
